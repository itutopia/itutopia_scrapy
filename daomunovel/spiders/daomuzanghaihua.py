...
class DaomushahaiSpider(scrapy.Spider):
    name = 'daomuzanghaihua'
    ...
    def parse(self, response):
        ...
        # 开始解析网页第一层，拿到书的名称
        # 沙海
        res = response.xpath('//li[@id="menu-item-2076"]/a')

        for i in res:
            item = DaomunovelItem()
            # 书的名称
            item['book_title'] = i.xpath('.//text()').get()
            # 拿到书连接，可以进入拿到书的章节
            url_one = i.xpath('.//@href').get()

            # 存放路径
            book_path = re.sub(r'[\\\/\:\*\?\"\<\>\|]', '_', item['book_title'])

            dirpath = './zanghaihua/{}'.format(book_path)
    ...
    def parse_two(self, response):
        ...
        for i in res_two:
            ...
            yield scrapy.Request(url_two, callback=self.parse_three, meta={'item': deepcopy(item)})
